...
- 최신 로컬 기준: `python -m pytest -q` = `94 passed`.
...
- Current verification baseline: 94 passed.
...
- `google_messages.py` internals are modularized into `google_messages_base.py` / `google_messages_auth.py` / `google_messages_chat.py` / `google_messages_processing.py`, with `GoogleMessagesPage` maintained as a facade for backward compatibility.
- Current verification baseline: 94 passed.
