...
def test_startup_target_is_empty_even_with_saved_setting(qtbot):
    from PyQt5.QtCore import QSettings

    st = QSettings("ImageMacro", "MVP")
    prev = st.value("general/target_window_title", None)
    st.setValue("general/target_window_title", "Notepad")
    try:
        mw = MainWindow()
        qtbot.addWidget(mw)
        assert mw.edTargetTitle.text() == ""
    finally:
        if prev is None:
            st.remove("general/target_window_title")
        else:
            st.setValue("general/target_window_title", prev)
...
def test_runner_default_target_title_is_empty():
    step = StepData(id="s0", name="noop", type="comment")
    runner = MacroRunner([step], repeat=RepeatConfig(repeat_count=1), dry_run=True, target_window_title=None)
    assert runner.target_window_title == ""
