...
def test_image_click_relative_target_search_clicks_target_with_ratio_area(monkeypatch, patched_runner):
    _runner_mod, _pg = patched_runner
    step = StepData(
        id="img-relative-ratio",
        name="Relative Ratio",
        type="image_click",
        timeout_ms=100,
        poll_ms=1,
        relative_target_enabled=True,
        relative_target_png_bytes=b"target-ratio",
        relative_search_mode="ratio",
        relative_search_right_ratio=10.0,
        relative_search_bottom_ratio=5.0,
    )
    runner = MacroRunner([step], repeat=RepeatConfig(), dry_run=False)
    clicked: dict[str, tuple[int, int]] = {}

    def fake_match(frame, current_step):
        if getattr(current_step, "png_bytes", None) == b"target-ratio":
            return SimpleNamespace(ok=True, x=60, y=10, score=0.97, w=12, h=8)
        return SimpleNamespace(ok=True, x=25, y=20, score=0.99, w=10, h=6)
