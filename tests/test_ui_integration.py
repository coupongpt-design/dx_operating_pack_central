...
def test_hotkey_pause_resume_toggles_runner(monkeypatch, qapp, qtbot):
    from app.main import MainWindow
    ...
    assert win.runner.resume_calls == 1
    assert win._macro_paused_ui is False
    assert "실행" in win.btnRun.text()
    win.close()


def test_hotkey_kill_calls_runner_kill_and_disables_stop(monkeypatch, qapp, qtbot):
    from app.main import MainWindow
    ...
    qtbot.addWidget(win)
    win.hide()
    win.runner = KillRunner()
    win.act_stop.setEnabled(True)
    win.btnStop.setEnabled(True)

    win._act_kill_from_hotkey()

    assert win.runner.kill_calls == ["global_hotkey"]
    assert win.act_stop.isEnabled() is False
    assert win.btnStop.isEnabled() is False
    win.close()


def test_hotkey_dialog_rejects_duplicate_shortcuts(qapp):
    from app.ui.hotkeys import HotkeySettingsDialog
    ...
    assert "이미 사용 중인 단축키" in dlg.lblConflict.text()
    dlg.close()


def test_hotkey_dialog_accepts_unique_shortcuts(qapp):
    from app.ui.hotkeys import HotkeySettingsDialog
    ...
    dlg.accept()
    assert dlg.result() == QDialog.Accepted
    assert len(dlg.result_hotkeys()) == 7
    dlg.close()
...
def test_runner_step_uuid_handlers_update_runtime_debug_ui(monkeypatch, qapp, qtbot):
    from app.main import MainWindow
    from app.core.models import StepData
    ...
    a = StepData(id="runtime-a", name="A", type="comment")
    b = StepData(id="runtime-b", name="B", type="comment")
    b.source_step_id = "source-b"
    win.steps = [a, b]
    win.refresh_step_list()

    win._on_runner_step_started("source-b", "B")
    assert win._active_step_index == 1
    assert "source-b" in win.lblRuntimeStep.text()

    win._on_runner_step_failed("source-b", "B", "boom")
    assert win._last_failed_step_index == 1
    assert win.list._failed_row == 1
    assert "boom" in win.lblRuntimeFail.text()
    win.close()


def test_open_execution_history_dialog(monkeypatch, qapp, qtbot):
    from app.main import MainWindow
    ...
    monkeypatch.setattr("app.main.ExecutionHistoryDialog", DummyHistoryDialog)

    win = MainWindow()
    qtbot.addWidget(win)
    win.hide()

    win._open_execution_history()

    assert called["parent"] is win
