...
def match_template_with_color(img_like: np.ndarray, tpl_like: np.ndarray, step: StepData, mask: np.ndarray | None = None) -> np.ndarray:
    def _match_single(src: np.ndarray, tpl: np.ndarray, mask_like: np.ndarray | None = None) -> np.ndarray:
        if mask_like is None:
            return cv2.matchTemplate(src, tpl, cv2.TM_CCOEFF_NORMED)
        try:
            return cv2.matchTemplate(src, tpl, cv2.TM_CCORR_NORMED, mask=mask_like)
        except cv2.error as e:
            LOGGER.debug("Masked template matching fallback without mask: %s", e)
            return cv2.matchTemplate(src, tpl, cv2.TM_CCOEFF_NORMED)

    use_color = bool(getattr(step, "match_color", False))
    if (not use_color or len(getattr(img_like, "shape", ())) < 3):
        return _match_single(img_like, tpl_like, mask)
    
    res = None
    for ch in range(3):
        channel_res = _match_single(img_like[:, :, ch], tpl_like[:, :, ch], mask)
        if res is None:
            res = channel_res
        else:
...
class Matcher:
    ...
    def _color_gate_passed(
        self,
        frame_like: np.ndarray,
        tpl_like: np.ndarray | None,
        x: int,
        y: int,
        step: StepData,
        mask_like: np.ndarray | None = None,
    ) -> bool:
        ...
        try:
            ...
            if mask_like is not None:
                if mask_like.shape[:2] != diff.shape[:2]:
                    return False
                m = mask_like > 0
                if not np.any(m):
                    return False
                if len(diff.shape) == 3:
                    m3 = np.repeat(m[:, :, np.newaxis], diff.shape[2], axis=2)
                    vals = diff[m3]
                else:
                    vals = diff[m]
                if vals.size == 0:
    ...
    def _mask_enabled(self, step: StepData) -> bool:
        return bool(getattr(step, "alpha_mask_enable", True)) or bool(getattr(step, "auto_fg_mask_enable", False))

    def _build_auto_fg_mask(self, tpl_bgr: np.ndarray | None, step: StepData | None = None) -> np.ndarray | None:
        if tpl_bgr is None or tpl_bgr.size == 0:
            return None
        if len(getattr(tpl_bgr, "shape", ())) != 3 or tpl_bgr.shape[2] < 3:
            return None
        h, w = tpl_bgr.shape[:2]
        if h < 3 or w < 3:
            return None
        try:
            ...
            left = tpl_bgr[:, 0, :]
            right = tpl_bgr[:, -1, :]
            border = np.vstack((top, bottom, left, right)).astype(np.float32)
            bg = np.median(border, axis=0)

            diff = tpl_bgr.astype(np.float32) - bg[np.newaxis, np.newaxis, :]
            dist = np.sqrt(np.sum(diff * diff, axis=2))
            pct = float(getattr(step, "auto_fg_mask_bg_percentile", 70.0) if step is not None else 70.0)
            pct = min(95.0, max(50.0, pct))
            dyn_scale = float(getattr(step, "auto_fg_mask_dynamic_scale", 0.6) if step is not None else 0.6)
            dyn_scale = min(2.0, max(0.1, dyn_scale))
            min_dist = float(getattr(step, "auto_fg_mask_min_distance", 12.0) if step is not None else 12.0)
            min_dist = min(255.0, max(1.0, min_dist))
            dyn = float(np.percentile(dist, pct))
            thresh = max(min_dist, dyn * dyn_scale)
            mask = np.where(dist >= thresh, 255, 0).astype(np.uint8)
            ...
            LOGGER.debug("Auto foreground mask generation skipped: %s", e)
            return None

    def _get_base_mask(self, step: StepData) -> np.ndarray | None:
        if not self._mask_enabled(step):
            return None
        use_alpha = bool(getattr(step, "alpha_mask_enable", True))
        if use_alpha:
            mask = getattr(step, "_tpl_mask", None)
            if mask is None:
                try:
                    step.ensure_tpl()
                except Exception:
        ...
        if not bool(getattr(step, "auto_fg_mask_enable", False)):
            return None
        ...
        try:
            ...
            tpl_bgr = None
        auto_mask = self._build_auto_fg_mask(tpl_bgr, step)
        cache[key] = auto_mask
    ...
    def _get_pp_mask(self, step: StepData, overrides: dict | None = None) -> np.ndarray | None:
        base_mask = self._get_base_mask(step)
        if base_mask is None:
            return None
        cache = getattr(step, "_tpl_cache", None)
        if cache is None:
            step._tpl_cache = cache = {}
        key = ("pp_mask", self._pp_key(step, overrides))
        if key in cache:
            val = cache.pop(key)
            cache[key] = val
            return val
    ...
    def _get_scale_mask(self, mask_pp, step, sc, overrides: dict | None = None):
        if mask_pp is None:
            return None
        cache = getattr(step, "_tpl_cache", None)
        if cache is None:
            step._tpl_cache = cache = {}
        key = ("scale_mask", self._pp_key(step, overrides), float(sc))
        if key in cache:
            val = cache.pop(key)
            cache[key] = val
            return val
        nh, nw = max(1, int(mask_pp.shape[0] * float(sc))), max(1, int(mask_pp.shape[1] * float(sc)))
        if nh < 2 or nw < 2:
            cache[key] = None
            return None
        mask = cv2.resize(mask_pp, (nw, nh), interpolation=cv2.INTER_NEAREST)
        mask = np.where(mask > 0, 255, 0).astype(np.uint8)
    ...
    def _get_rot_mask(self, mask_pp, step, ang, overrides: dict | None = None, extra_key=None):
        if mask_pp is None:
            return None
        cache = getattr(step, "_tpl_cache", None)
        if cache is None:
            step._tpl_cache = cache = {}
        key = ("rot_mask", self._pp_key(step, overrides), float(ang))
        if extra_key is not None:
            key = ("rot_mask", self._pp_key(step, overrides), extra_key, float(ang))
        if key in cache:
            val = cache.pop(key)
            cache[key] = val
            return val
        ...
        rot = cv2.warpAffine(mask_pp, M, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
        rot = np.where(rot > 0, 255, 0).astype(np.uint8)
    ...
    def _quality_passes(self, step: StepData) -> list[tuple[str, dict | None]]:
        ...
        use_color = bool(getattr(step, "match_color", False))
        if use_color:
            if not bool(getattr(step, "hq_color_bg_robust", False)):
                return passes
            # Optional fallback for color mode: retry robust grayscale passes
            # with color matching disabled to survive large background shifts.
            add_pass("gray_fallback", {"force_gray": True, "pre_gray": True, "match_color": False})
            add_pass(
                "clahe_fallback",
                {"force_gray": True, "pre_gray": True, "pre_clahe": True, "pre_sharpen": True, "match_color": False},
            )
            add_pass("edge_fallback", {"force_gray": True, "pre_gray": True, "pre_edge": True, "match_color": False})
            return passes
    ...
    def find_best_optimized(self, frame_bgr: np.ndarray, step: StepData) -> MatchResult:
        ...
        try:
            ...
            def color_gate_passed(
                ...
                if overrides:
                    ...
                    if "match_color" in overrides and not overrides["match_color"]:
                        return True
                return self._color_gate_passed(img_like, tpl_like, x, y, step, tpl_mask_like)
            ...
            for pass_name, overrides in self._quality_passes(step):
                ...
                tpl_mask_pp = self._get_pp_mask(step, overrides)

                # 1. Single Scale
                t0 = time.perf_counter()
                res = match_template_with_color(img_pp, tpl_pp, step, tpl_mask_pp)
                cands = _extract_topk(res, k, tpl_pp.shape[1], tpl_pp.shape[0])
                add_timing("single_ms", t0)

                for (score, x, y) in cands:
                    if not color_gate_passed(img_pp, tpl_pp, tpl_mask_pp, x, y, overrides):
                        continue
                    if high_mode:
                        record_best(
                            score,
                ...
                if enable_scale:
                    ...
                    for sc in scales:
                        ...
                        if tpl_s is None:
                            continue
                        tpl_mask_s = self._get_scale_mask(tpl_mask_pp, step, sc, overrides)

                        try:
                            res = match_template_with_color(img_pp, tpl_s, step, tpl_mask_s)
                            cands = _extract_topk(res, k, tpl_s.shape[1], tpl_s.shape[0])
                            for (score, x, y) in cands:
                                if not color_gate_passed(img_pp, tpl_s, tpl_mask_s, x, y, overrides):
                                    continue
                                if high_mode:
                                    record_best(
                                        score,
                ...
                if enable_rot:
                    ...
                    for ang in angles:
                        ...
                        tpl_mask_r = self._get_rot_mask(tpl_mask_pp, step, ang, overrides)
                        try:
                            res = match_template_with_color(img_pp, tpl_r, step, tpl_mask_r)
                            cands = _extract_topk(res, k, tpl_r.shape[1], tpl_r.shape[0])
                            for (score, x, y) in cands:
                                if not color_gate_passed(img_pp, tpl_r, tpl_mask_r, x, y, overrides):
                                    continue
                                if high_mode:
                                    record_best(
                                        score,
                ...
                if enable_combo:
                    ...
                    for sc in scales:
                        ...
                        for ang in angles:
                            ...
                            tpl_mask_sr = self._get_rot_mask(
                                tpl_mask_s,
                                step,
                                ang,
                                overrides,
                            ...
                            try:
                                res = match_template_with_color(img_pp, tpl_sr, step, tpl_mask_sr)
                                cands = _extract_topk(res, k, tpl_sr.shape[1], tpl_sr.shape[0])
                                for (score, x, y) in cands:
                                    if not color_gate_passed(img_pp, tpl_sr, tpl_mask_sr, x, y, overrides):
                                        continue
                                    record_best(
                                        score,
                                        x + tpl_sr.shape[1] // 2,
    ...
    def find_all(self, frame_bgr: np.ndarray, step: StepData) -> list[tuple[int, int, float]]:
        ...
        tpl_mask_pp = self._get_pp_mask(step, None)
        ...
        for (x1, y1, x2, y2, score) in suppressed_boxes:
            if not self._color_gate_passed(img_pp, tpl_pp, x1, y1, step, tpl_mask_pp):
                continue
            center_x = x1 + tpl_w // 2
            center_y = y1 + tpl_h // 2
