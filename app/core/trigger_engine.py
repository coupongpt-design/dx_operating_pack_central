...
class TriggerWatcher(QThread):
    ...
    def stop(self, timeout_ms: int = 2000) -> bool:
        ...
        self.log.emit(f"[Trigger] Stop timeout after {timeout}ms; forcing thread termination.")
        try:
            self.terminate()
            self.wait(200)
        except Exception as e:
            self.log.emit(f"[Trigger] Force terminate failed: {e}")
    ...
    def _should_stop(self) -> bool:
        return bool(self._stop or self._stop_event.is_set() or self.isInterruptionRequested())

    def _wait_or_wake(self, timeout_sec: float):
        self._wake_event.wait(max(0.0, float(timeout_sec)))
    ...
    def run(self):
        ...
        with mss.mss() as sct:
            ...
            while not self._should_stop():
                try:
                    if self._paused:
                        self._wait_or_wake(self._poll_interval_sec)
                        continue
