...
class MacroRunner(QThread):
    ...
    def __init__(
        ...
        if target_window_title is None:
            try:
                ...
                target_window_title = st.value("general/target_window_title", "", type=str)
            except Exception:
    ...
    def _image_click(self, sct, mon, s: StepData, idx: int) -> tuple[bool, str | None, int]:
        ...
        while (time.time() - t0) * 1000 <= int(s.timeout_ms):
            ...
            if mr.ok:
                ...
                s._last_match_xy = (cx, cy)
                if self.dry_run:
                    self.requestCrosshair.emit(cx, cy, 200)
                else:
                    self._perform_click(cx, cy, s)
        ...
        while (time.time() - t0) * 1000 <= int(step.timeout_ms):
            ...
            for target in step.conditional_targets:
                ...
                if mr.ok:
                    ...
                    if not self.dry_run:
                        self._perform_click(cx, cy, temp_step)
                    else:
                        self.requestCrosshair.emit(cx, cy, 200)
    ...
    def _load_data_file(self, s: StepData) -> bool:
        ...
        success, data, error = load_csv(path)
        ...
        # Convert list of dicts to list of lists (values only) for compatibility if needed,
        # OR update usage to use dicts.
        # The existing code uses `_data_row_values` as list of lists.
        # Let's maintain compatibility.
        
        self._data_row_values = [list(row.values()) for row in data]
        
        # _data_list is used for simple iteration, usually the first column
        self._data_list = [list(row.values())[0] for row in data if row]
        
        self._data_index = 0
        self.log.emit(f"  -> Loaded data file: {len(data)} rows.")
        return True
