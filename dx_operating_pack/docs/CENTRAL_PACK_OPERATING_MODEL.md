...
- 여러 프로젝트의 `dx_operating_pack` 운영 방식을 통일할 때
- 중앙 PACK repo를 운영할 때
- feedback inbox/outbox/promotion 흐름을 정리할 때
- 로컬 PACK과 중앙 PACK의 관계가 헷갈릴 때

기본 원칙:

- 일반 제품 기능 작업의 baseline 문서로 읽지 않는다.
- PACK 설치, 동기화, 수확물 상신, 중앙 승격 같은 DX 운영 작업에서만 읽는다.
